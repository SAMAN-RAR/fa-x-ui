...
const ImageEditor = (props: ImageEditorPropsType) => {
  ...
  const changeSensitiveHandler = (sensitive: boolean) => {
    setSettings((prevState) => ({ ...prevState, sensitive: sensitive }));
  };

  const changeTypeHandler = (type: "original" | "square" | "wide") => {
    setSettings((prevState) => ({ ...prevState, type: type }));
  };
      ...
      <div className="bg-black rounded-xl p-12 flex flex-col gap-4">
        ...
        <div className="w-[600px] h-[600px] flex items-center">
          <Image
            ...
            width={600}
            height={600}
            className={`w-full object-cover ${imageClasses}`}
          />
        </div>
        <div className="flex justify-between items-center text-sm">
          <div className="flex items-center gap-8">
            <div
              ...
              onClick={() => changeTypeHandler("original")}
            >
            ...
            <div
              className="flex items-center gap-2 cursor-pointer"
              onClick={() => changeTypeHandler("square")}
            >
              <svg width={24} viewBox="0 0 24 24">
                <path
                  className={
              ...
              <span
                className={
                  settings.type === "square" ? "text-iconBlue" : "text-textGray"
                }
              >
                مربعی
              </span>
            ...
            <div
              className="flex items-center gap-2 cursor-pointer"
              onClick={() => changeTypeHandler("wide")}
            >
              <svg width={24} viewBox="0 0 24 24">
                <path
                  className={
              ...
              <span
                className={
                  settings.type === "wide" ? "text-iconBlue" : "text-textGray"
                }
              >
                عریض
              </span>
          ...
          <div
            className={`cursor-pointer py-1 px-4 rounded-full text-white ${
              settings.sensitive ? "bg-red-500" : "bg-slate-700"
            }`}
            onClick={() => changeSensitiveHandler(!settings.sensitive)}
          >
            محتوا حساس
          </div>
        </div>
